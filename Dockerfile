...
# install build dependencies (Node 22.x from NodeSource for npm compatibility)
RUN apt-get update \
    && apt-get install -y --no-install-recommends build-essential git ca-certificates curl gnupg \
...
# install npm dependencies (needed before assets.setup runs npm install)
